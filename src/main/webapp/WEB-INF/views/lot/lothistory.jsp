	...
	<form method="post"
		...
		<div class="coTableTop">
			...
			<p class="coTotalCount">
				총 ${empty pageInfo.totalCount ? 0 : pageInfo.totalCount}건
			</p>
				...
				<thead>
					<tr>
						<th class="mobile_show">
							<label id="lotHistoryCheckAllLabel">선택</label>

							<input type="checkbox"
								id="lotHistoryCheckAll"
								style="display: none;">
						</th>
						...
						<tr>
							...
							<td class="mobile_hidden">
								<c:choose>
									...
									<c:when test="${lot.inspResult eq '조건부' or lot.inspResult eq '조건부 합격'}">
										<span class="coStatus coStatusWait">
											${lot.inspResult}
										</span>
									</c:when>
									...
									<c:otherwise>
										<span class="coStatus">
											${empty lot.inspResult ? '-' : lot.inspResult}
										</span>
									</c:otherwise>
								</c:choose>
							</td>
							...
							<td class="mobile_show">
								<c:choose>
									...
									<c:otherwise>
										<span class="coStatus">
											${empty lot.progressStatus ? '-' : lot.progressStatus}
										</span>
									</c:otherwise>
								</c:choose>
							</td>
...
<script>
	// LOT 이력추적 목록의 전체 선택 라벨을 찾는다.
	var lotHistoryCheckAllLabel = document.getElementById("lotHistoryCheckAllLabel");

	// 전체 선택 라벨이 있을 때만 이벤트를 연결한다.
	if (lotHistoryCheckAllLabel != null) {

		// 선택 라벨을 클릭했을 때 전체 선택 상태를 변경한다.
		lotHistoryCheckAllLabel.onclick = function() {

			// 숨겨진 전체 선택 체크박스를 찾는다.
			var checkAll = document.getElementById("lotHistoryCheckAll");

			// 전체 선택 체크박스가 없으면 실행하지 않는다.
			if (checkAll == null) {
				return;
			}
			...
			var checkboxList = document.querySelectorAll("#lotHistoryForm input[name='orderIds']");

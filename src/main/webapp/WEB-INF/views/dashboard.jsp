...
<div class="dash-alert-items${empty dashIssueTotalCount or dashIssueTotalCount le 0 ? ' dash-alert-items-empty' : ''}">
	...
	<c:if test="${dashDefectIssueYn eq 'Y'}">
		<div class="dash-alert-card">
			<div class="dash-alert-text">
				...
				<span class="dash-alert-desc">
					불량률 <fmt:formatNumber value="${dashDefectIssueRate}" pattern="#,##0.0" />%
					<span class="dash-alert-desc-sub">
						(기준 <fmt:formatNumber value="${dashDefectStandardRate}" pattern="#,##0.0" />% 초과)
					</span>
				</span>
			</div>
		</div>
	...
	<c:if test="${dashTroubleIssueYn eq 'Y'}">
		<div class="dash-alert-card">
			<div class="dash-alert-text">
				...
				<span>
					미조치 설비 고장
					<fmt:formatNumber value="${dashTroubleCount}" pattern="#,##0" />건 발생
				</span>
			</div>
		</div>
	</c:if>
	...
	<c:if test="${dashDelayIssueYn eq 'Y'}">
		<div class="dash-alert-card dash-alert-orange">
			<div class="dash-alert-text">
				...
				<span>
					지연 작업지시
					<fmt:formatNumber value="${dashDelayOrderCount}" pattern="#,##0" />건 발생
				</span>
			</div>
		</div>
	</c:if>
...
<div class="dash-lot-row">
	<div class="dash-lot-box" onclick="location.href='${pageContext.request.contextPath}/lot/lothistory'">
	...
	<div class="dash-lot-box" onclick="location.href='${pageContext.request.contextPath}/lot/lothistory'">
		...
		<div class="dash-lot-info">
			<span>검사대기 LOT</span>
			<strong class="dash-orange-text">
				<c:choose>
					<c:when test="${empty dashInspectionWaitLotCount}">
						0
					</c:when>
					<c:otherwise>
						<fmt:formatNumber value="${dashInspectionWaitLotCount}" pattern="#,##0" />
					</c:otherwise>
				</c:choose>건
			</strong>
		</div>
	...
	<div class="dash-lot-box" onclick="location.href='${pageContext.request.contextPath}/lot/lothistory'">
		...
		<div class="dash-lot-info">
			...
			<strong class="dash-blue-text">
				<c:choose>
					<c:when test="${empty dashFinishedShipWaitLotCount}">
						0
					</c:when>
					<c:otherwise>
						<fmt:formatNumber value="${dashFinishedShipWaitLotCount}" pattern="#,##0" />
					</c:otherwise>
				</c:choose>건
			</strong>
		</div>
		...
		<tbody>
			<c:choose>
				<c:when test="${empty dashDefectTopList}">
					<tr>
						<td colspan="4">조회된 불량 정보가 없습니다.</td>
					</tr>
				</c:when>

				<c:otherwise>
					<c:forEach var="defect" items="${dashDefectTopList}" varStatus="status">
						<tr>
							<td>${status.count}</td>
							...
							<td>
								<fmt:formatNumber value="${defect.defect_qty}" pattern="#,##0" />
							</td>

							<td>
								<span class="dash-bar">
									<em style="width: ${defect.sort_no}%;"></em>
								</span>
							</td>
						</tr>
					</c:forEach>
				...
				<c:otherwise>
					<c:forEach var="defect" items="${dashDefectTopList}" varStatus="status">
						<tr>
							...
							<td>
								<fmt:formatNumber value="${defect.sort_no}" pattern="#,##0" />%
							</td>
						</tr>
					</c:forEach>
				</c:otherwise>

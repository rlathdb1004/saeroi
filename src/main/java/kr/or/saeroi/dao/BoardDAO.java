...
public interface BoardDAO {
	...
	// 게시판 목록
	List<BoradDTO> _dao_select_Board(String startDate, String endDate, String keyword);

	// 다음 게시판 번호 조회
	int _dao_select_next_Board_id();

	// 게시판 등록
	int _dao_insert_Board(int board_id, String title, String content, String empno,
			String status, String remark);

	// 게시판 첨부파일 등록
	int _dao_insert_Board_file(int board_id, String file_title, String saved_title,
			String file_path, long file_size);

	// 게시판 삭제
	int _dao_delete_Board(String[] board_id, String role, String empno);

	// 게시판 상세 조회
	BoradDTO _dao_select_Board_detail(String board_id, String role);

	// 게시판 첨부파일 조회
	BoradDTO _dao_select_Board_file(String board_id);

	// 게시판 조회수 증가
	int _dao_update_Board_view_count(String board_id, String empno);
